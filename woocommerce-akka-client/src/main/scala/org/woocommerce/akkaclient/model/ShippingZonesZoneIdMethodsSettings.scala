/**
 * wc/v3
 * OpenAPI documented, based on generated OpenAPI document of the namespace wc/v3 with some manual fixes.
 *
 * The version of the OpenAPI document: 1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
package org.woocommerce.akkaclient.model

import org.woocommerce.akkaclient.core.ApiModel

case class ShippingZonesZoneIdMethodsSettings (
  /* A unique identifier for the setting. */
  id: Option[String] = None,
  /* A human readable label for the setting used in interfaces. */
  label: Option[String] = None,
  /* A human readable description for the setting used in interfaces. */
  description: Option[String] = None,
  /* Type of setting. */
  `type`: Option[ShippingZonesZoneIdMethodsSettingsEnums.`Type`] = None,
  /* Setting value. */
  value: Option[String] = None,
  /* Default value for the setting. */
  default: Option[String] = None,
  /* Additional help text shown to the user about the setting. */
  tip: Option[String] = None,
  /* Placeholder text to be displayed in text inputs. */
  placeholder: Option[String] = None
) extends ApiModel

object ShippingZonesZoneIdMethodsSettingsEnums {

  type `Type` = `Type`.Value
  object `Type` extends Enumeration {
    val Text = Value("text")
    val Email = Value("email")
    val Number = Value("number")
    val Color = Value("color")
    val Password = Value("password")
    val Textarea = Value("textarea")
    val Select = Value("select")
    val Multiselect = Value("multiselect")
    val Radio = Value("radio")
    val ImageWidth = Value("image_width")
    val Checkbox = Value("checkbox")
    val `Class` = Value("class")
    val Order = Value("order")
  }

}
